...
class RealestateHomeController extends Controller
{
    ...
    public function index(){
        $sliders =Slider::orderBy('id','asc')->where([['modelo','realestate'],['status',1]])->get();
        $aboutUs = About::where('modelo','realestate')->first();
        ...
        $realestateservices=RealestateService::where('status',1)->get();
        $websiteLang=ManageText::all();

        return view('realestate.index',compact('sliders','aboutUs', 'overviews','properties','sections','services','currency','seo_text','service_bg','agents','orders','blogs','default_profile_image','testimonials','propertyTypes','cities','feature_image','testimonial_bg','agent_bg','contact','contactSetting','realestateservices','websiteLang'));
    }
